...
## Latest Practice Highlight — Rising Temperature

The latest problem reinforced an important distinction between **row order** and **calendar continuity**.

Two SQL approaches were explored:

1. **Self Join + `DATEDIFF()`** — directly match each date with the previous calendar day.
2. **`LAG()` + `DATEDIFF()`** — use a window function to access the previous row, then verify that the row is exactly one day earlier.

The Pandas recreation uses:
...
> **Previous row does not necessarily mean previous calendar day. Always validate the date gap when the business rule says “yesterday.”**
...
A useful lesson from **Rising Temperature** is that the previous row is not always the previous calendar day.
...
After sorting by date, `2026-08-01` is the previous row for `2026-08-03`, but it is **not yesterday**.

Therefore, row-navigation logic must be paired with date-gap validation:

```text
SQL
LAG() -> get the previous row
DATEDIFF() -> verify the gap is exactly one day

Pandas
shift(1) -> get the previous row
datetime subtraction + pd.Timedelta(days=1) -> verify the one-day gap
```

This prevents incorrect comparisons when one or more calendar dates are missing.
