...
\| 2026-08-15 | Second Highest Salary | Subquery, \`DENSE\_RANK()\` | Filtering, \`max()\` |
...
\| \`RANK()\` / \`ROW\_NUMBER()\` | \`rank()\` with the appropriate method / ordering logic |
...
\| \`WHERE\` filtering | Boolean filtering / \`query()\` |
...
def second\_highest\_salary(

    employee: pd.DataFrame

) -> pd.DataFrame:

    max\_salary = employee["salary"].max()

    filtered\_employee = employee[

        employee["salary"] < max\_salary

    ]

    second\_salary = filtered\_employee["salary"].max()
